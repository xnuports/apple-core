 ...
 *
 * darwintools note: vendored from Apple's Libc (libdarwin/h/errno.h, installed
 * as <os/errno.h>), which is absent from the public macOS SDK.  The libdarwin-
 * private includes (<_bounds.h>, <os/api.h>) and the OS_EXPORT /
 * DARWIN_API_AVAILABLE attributes on sysexit_np have been dropped so the header
 * is self-contained against the stock SDK; the API surface (the *_NP error
 * codes and sysexit_np) is unchanged.
 */
 ...
 * appropriate exit code could be determined, the designated exit
